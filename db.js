...
async function addComment(post_info) {

    console.log('in add comment db')
    console.log(post_info)

    const db = await connectDB('Posts')

    const data = {$set: {comment: post_info.comment}}

    console.log(data)

    let addedComment = await db.updateOne({title: post_info.post_title}, data)
...
module.exports = { model, connectDB, addComment, addToDB, addPost, findClub, findUser, addToFavorites, deletePost, findPost, findFavorites, editPost, getPosts, findUserSecurityInfo }
